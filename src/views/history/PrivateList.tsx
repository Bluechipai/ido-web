import {useQueryUserPrivateList} from '../../api/user.ts'
...
export default function PrivateList(props: IProps) {
  const userStore = useAppSelector((state) => state.users)
  const [page, setPage] = useState(1);
  const [pageSize, setPageSize] = useState(10);

  const {data, isPending, queryClient} = useQueryUserPrivateList({
    page: page,
    pageSize: pageSize,

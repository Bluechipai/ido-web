...
export async function getBalance(address: string) {
    const balance = await getWallet().provider.getBalance(address);
...
export function extendTran(data: ITrans, info: {type: string, symbol?: string, status: number}):ITrans  {
    const transData = Object.assign({}, data);
...
export function divDecimals(amount:any, decimals:number){
    return Decimal.div(amount.toString(), Math.pow(10, decimals));
}
...
export function getTokenNameByAddress(address: string) {
    let name = "";
    Object.keys(project.contracts).some((item) => {
      // @ts-ignore
        if (project.contracts[item].address === address) {
          name = item;
          return true;
        }
    })
